...
async function startVoice() {
    ...
    if (isRecording) {
        mediaRecorder.stop();
        isRecording = false;
        if (btn) btn.classList.remove("bg-white", "text-black", "animate-pulse");
        if (btn) btn.classList.add("text-text-secondary");
    ...
    try {
        ...
        mediaRecorder.onstop = async () => {
            ...
            reader.onloadend = async function () {
                ...
                try {
                    const response = await fetch(`${getSteelApiBase()}/transcribe`, {
                        method: "POST",
        ...
        if (btn) {
            ...
            btn.classList.add("text-white", "animate-pulse");
        }
